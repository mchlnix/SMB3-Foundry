...
def test_find():
    rom_bytes = bytearray(
        b"\x00\x01\x02\x03\x04\x05\x06\x00\xff\xff\xff\xff\xff\xff\xff\xff"
    )
...
def test_int():
    rom_bytes = bytearray(
        b"\x00\x01\x02\x03\x04\x1f\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff"
    )
    header = INESHeader.from_buffer_copy(rom_bytes)
...
def test_expanded_rom_reads_same_as_normal(rom, expanded_rom):
    ...
    for offset in range(
        0, rom._header.prg_size + rom._header.chr_size, INESHeader.PRG_UNIT_SIZE // 4
    ):
        if offset == 0:
