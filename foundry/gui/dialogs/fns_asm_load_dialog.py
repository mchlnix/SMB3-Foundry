...
class FnsAsmLoadDialog(CustomDialog):
    def __init__(self, parent, cur_fns_file: str = "", current_asm_file: str = ""):
        ...
        explanation = QLabel(
            _("Provide an FNS file and the smb3.asm file from your project.")
        )
        ...
        question_label.setPixmap(
            self.style().standardPixmap(QStyle.StandardPixmap.SP_MessageBoxInformation)
        )
        ...
        self._fns_check_icon.setPixmap(
            self.style().standardPixmap(QStyle.StandardPixmap.SP_MessageBoxCritical)
        )

        self._fns_open_button = QPushButton()
        self._fns_open_button.pressed.connect(self._get_fns_file)
        self._fns_open_button.setIcon(
    ...
    def _check_fns_file(self, path: str):
        ...
        if not new_path.is_file():
            ...
            self._fns_check_icon.setToolTip(
                _("Given path is not a file/does not exist.")
            )
            return
        ...
        try:
            ...
            self._fns_check_icon.setPixmap(
                self.style().standardPixmap(QStyle.StandardPixmap.SP_MessageBoxWarning)
            )
        ...
        self._fns_check_icon.setPixmap(
            self.style().standardPixmap(QStyle.StandardPixmap.SP_DialogYesButton)
        )
        self._fns_check_icon.setToolTip("")
    ...
    def _check_fns_content(new_path: Path):
        ...
        for line in new_path.open().readlines():
            ...
            try:
                ...
                raise ValueError(
                    _(
                        "Didn't find lines in the form of 'name = $1234'. File might be wrongly formatted."
                    )
                )
    ...
    def _get_fns_file(self):
        fns_file, __ = QFileDialog.getOpenFileName(
            self, _("Open FNS File"), filter=FNS_FILE_FILTER
        )
    ...
    def _get_asm_file(self):
        asm_file, __ = QFileDialog.getOpenFileName(
            self, _("Open smb3.asm File"), filter=SMB3_ASM_FILE_FILTER
        )
