...
class BoomBoomMixin(SettingsMixin):
    def __init__(self, parent):
        ...
        self.boom_boom_dropdown.addItems(
            [
                f"{boom_boom.name} at {boom_boom.get_position()}"
        ...
        boom_boom_group.layout().addLayout(
            label_and_widget(_("Lock index"), self.boom_boom_index_spinner)
        )
    ...
    def _on_boom_boom_spinner(self, new_value):
        boom_boom = _get_boom_booms(self.level_ref.enemies)[
            self.boom_boom_dropdown.currentIndex()
        ]
