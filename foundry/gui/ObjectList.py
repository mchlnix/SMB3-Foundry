...
class ObjectList(QListWidget):
    ...
    selection_changed : SignalInstance
        Signal emitted with the selected object payloads after list selection
        changes. Rows keep their stable object references in ``Qt.UserRole``;
        localized labels are regenerated by :meth:`retranslate_ui` and never
        become selection, undo, or command identity.
    """
    ...
    def update_content(self):
        ...
        Row text is localized for display only; each row's ``Qt.UserRole``
        payload remains the object used for selection, context menus, and
        ...
        for index, level_object in enumerate(level_objects):
            ...
            if (item := self.item(index)) is None:
                self.insertItem(index, tr_object_name(level_object))
            ...
            if level_object != item.data(Qt.ItemDataRole.UserRole):
                item.setText(tr_object_name(level_object))
    ...
    def retranslate_ui(self) -> None:
        """Refresh visible row text without changing stored object payloads.

        Each row stores the underlying level object or enemy in
        ``Qt.UserRole``. Live translation replaces only the label generated by
        :func:`tr_object_name`, preserving row selection, context-menu targets,
        object identity, and later undo/placement behavior.
        """
        self.setWhatsThis(
            tr(
                TR_CONTEXT,
                "help.object_list",
                "<b>Object List</b><br/>This lists all the objects and enemies/items in the level. They appear in the order, that they are stored in the ROM as, which also decides which objects get drawn before/behind which.<br/>Enemies/items are always listed last, since they are also stored separately from the level objects.<br/><br/>Note: While Jumps are technically level objects, they are omitted here, since they are listed in a separate list below.",
            )
        )
        for index in range(self.count()):
            item = self.item(index)
            level_object = item.data(Qt.ItemDataRole.UserRole)
