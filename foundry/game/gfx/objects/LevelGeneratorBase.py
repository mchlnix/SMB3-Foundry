...
class SlopeGenerator(BlockGeneratorBase):
    def to_bytes(self) -> Tuple[int, bytearray]:
        ...
        return tileset, bytearray([domain << 5 + y, x, (index & 0xFFF0) + width])
...
class VerticalGenerator(BlockGeneratorBase):
    def to_bytes(self) -> Tuple[int, bytearray]:
        ...
        if self.generator.bytes == 3:
            return tileset, bytearray([domain << 5 + y, x, (index & 0xFFF0) + height])
        elif self.generator.bytes == 4:
            return tileset, bytearray([domain << 5 + y, x, (index & 0xFFF0) + width, height])
        else:
...
class HorizontalGenerator(BlockGeneratorBase):
    def to_bytes(self) -> Tuple[int, bytearray]:
        ...
        if gen.bytes == 3:
            ...
            return tileset, bytearray([domain << 5 + y, x, (index & 0xFFF0) + height, width])
        else:
            return NotImplemented
